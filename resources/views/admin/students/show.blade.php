            ...
            <table style="width: 100%; border-collapse: collapse; text-align: center;">
                ...
                <tbody>
                    @foreach ($intelligenceTypes as $id => $type)
                        @php
                            $pre_score_key = 'score_' . \Illuminate\Support\Str::of($type->name)->snake()->explode('_')->last();
                        ...
                        <tr>
                            ...
                            <td style="padding: 10px; border: 1px solid #ddd; font-weight: bold; font-size: 1.1em;">{{ $result->{$pre_score_key} ?? 'N/A' }}</td>
                            <td style="padding: 10px; border: 1px solid #ddd; font-weight: bold; font-size: 1.1em;">{{ $result->{$post_score_key} ?? 'لم يجرى بعد' }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
